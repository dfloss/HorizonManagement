...
Describe "Connect-HVServerList" -Tag "Unit"{
    ...
    Context "Happy Path"{
        ...
        It "works with a Credential file"{
            $Script:Config.CredentialFile = "NotNull"
            Connect-HvServerList
            Assert-MockCalled -CommandName 'Import-Clixml'
        }
    }
    Context "Sad Path"{
        It "Throws an error when an invalid credential file is found"{
            $Script:Config.CredentialFile = "TestDrive:\NotThere.txt"
            {Connect-HvServerList -ServerHash $TestHash} | Should Throw
        }
    }
}
